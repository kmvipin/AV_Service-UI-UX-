...
e.preventDefault();
  ...
  fetch('http://localhost:8080/api/service/addService',{
    method : 'POST',
    headers : {
    ...
    <>
     <div className="add-service-modal" style={{ display:"block"}}>
      <form className="addform">
        <span className="close-button" onClick={()=>{setShowAddServiceForm(false)}}>&times;</span>
          <h2 className="addheading">Add Your Service</h2>
          <label for="cars">Choose Your Category:</label>
           <select id="category" name="category" onChange={(e)=>setCategoryName(e.target.value)}>
            ...
            <input type="text" id="service-name" value = {serviceName} onChange={(e)=>setServiceName(e.target.value)} required></input>
          
            <label for="service-description">Description:</label>
            <textarea id="service-description" value = {msg} onChange={(e)=>setMsg(e.target.value)} required></textarea>
          
            <label for="service-price">Price:</label>
            <input type="text" id="service-price" value = {unit} onChange={(e)=>setUnit(e.target.value)} required></input>
